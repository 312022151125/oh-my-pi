...
import { USER_AGENT, OPENCODE_USER_AGENT, OPENCODE_CLIENT } from "@oh-my-pi/pi-utils";
...
describe("opencode-go usage provider", () => {
	...
	it("sends Authorization: Bearer <key> to the fixed usage route", async () => {
		...
		// Attribution headers: full opencode CLI wire format (required from 09/06).
		expect(calls[0]?.headers["User-Agent"]).toBe(OPENCODE_USER_AGENT);
		expect(calls[0]?.headers["x-opencode-client"]).toBe(OPENCODE_CLIENT);
		expect(typeof calls[0]?.headers["x-opencode-session"]).toBe("string");
		expect(calls[0]?.headers["x-opencode-session"]?.startsWith("ses_")).toBe(true);
